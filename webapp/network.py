...
def get_lan_ipv4_addresses() -> list[str]:
    """Return local IPv4 addresses useful for LAN access."""
    found: set[str] = set()
    ...
    # Route-based discovery: ask OS which local IP would be used for outbound LAN traffic.
...
def print_access_urls(host: str, port: int) -> None:
    print("=" * 60)
    print("ИТ-мастерская: веб-сервер запущен")
    ...
    if lan_urls:
        ...
        print("LAN IP не определён автоматически.")
        print("Узнайте IP этого ПК (ipconfig) и откройте http://<IP>:8000")
    if host not in {"0.0.0.0", "::"}:
        print(f"Внимание: сервер слушает только {host}, для сети нужен 0.0.0.0")
    print("Если с другого ПК не открывается — разрешите порт в брандмауэре Windows.")
    print("=" * 60)
