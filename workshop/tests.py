...
class AuthAndPagesTests(TestCase):
    ...
    def test_brand_logo_and_favicon(self):
        ...
        base = Path(__file__).resolve().parent / "static" / "workshop" / "img"
    ...
    def test_tv_ads_page(self):
        ...
        self.assertContains(r, "ПО и чат-боты на заказ")
    ...
    def test_sms_admin_and_debt_send(self):
        ...
        self.assertContains(r, "Плакат для зоны")
        # Одна строка масс-рассылки + одна одиночная.
        self.assertEqual(r.content.decode().count("Масс-рассылка"), 1)
        r = self.http.get("/marketing/poster")
        self.assertEqual(r.status_code, 200)
        self.assertContains(r, "Подключите уведомления")
        self.assertContains(r, "Сканируйте QR")
        self.assertContains(r, "Печать плаката")
        self.assertContains(r, cfg.bot_link)
